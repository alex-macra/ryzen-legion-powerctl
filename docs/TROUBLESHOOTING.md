...
Setting limits can work without the module if `/dev/mem` is permitted. RyzenAdj 0.19 uses the module only if its `drv_version` is compatible. Once it selects the module, it requires `smn`, `pm_table_size`, and `pm_table`; if any is missing, initialization fails without a `/dev/mem` fallback. A failed apply must not be treated as an unverified success. A successful apply may still print `Could not read the limits back ... this apply is unverified.` when metrics cannot be read. The module exposes `/sys/kernel/ryzen_smu_drv`, not `/dev/ryzen_smu_drv`. [Fire Range PM-table support remains incomplete](https://github.com/amkillam/ryzen_smu/issues/49).

`legion-powerctl doctor` distinguishes a usable RyzenAdj module interface, missing initialization files, a loaded module that failed to expose its interface, and an unloaded module. For missing files or a missing interface, check `sudo dmesg | grep -i ryzen_smu` for the actual probe error. `./install.sh --install-ryzen-smu` installs and loads the optional `ryzen_smu-dkms-git` module and sets it to load at boot when the required files exist; see [INSTALL.md](INSTALL.md) for the lockdown and Secure Boot caveats. Installing it cannot by itself add an unsupported PM-table version.
